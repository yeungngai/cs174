...
class Game:
    def __init__(self, surface):
        ...
        self.dot = Ball('white', 5, [250, 200], [6, 3], self.surface)
        self.color = pygame.Color('white')
        self.rect_width = 10
        self.rect_height = 40
        self.left_rect_x = 105
        self.left_rect_y = 180
        self.right_rect_x = 395
        self.right_rect_y = 180
        left_rect = pygame.Rect(self.left_rect_x, self.left_rect_y, self.rect_width, self.rect_height)
        right_rect = pygame.Rect(self.right_rect_x, self.right_rect_y, self.rect_width, self.rect_height)
    ...
    def handle_events(self):
        ...
        if pressed[pygame.K_q] and left_rect.top >= 0:
            left_rect.move_ip(0, -7)
        if pressed[pygame.K_a] and left_rect.bottom <= 400:
            left_rect.move_ip(0, 7)
        if pressed[pygame.K_p] and right_rect.top >= 0:
            right_rect.move_ip(0, -7)
        if pressed[pygame.K_l] and right_rect.bottom <= 400:
            right_rect.move_ip(0, 7)
    ...
    def draw(self):
        ...
        self.dot.draw()
        pygame.draw.rect(self.surface, self.color, left_rect)
        pygame.draw.rect(self.surface, self.color, right_rect)
...
class Ball:
    ...
    def left_paddle_collision(self):
        if left_rect.collidepoint(self.center[0], self.center[1]):
            return True

    def right_paddle_collision(self):
        if right_rect.collidepoint(self.center[0], self.center[1]):
            return True
